...
/*   every Client and every Channel.                                          */
/*                                                                            */
/* ************************************************************************** */
...
class CommandDispatcher
{
	public:
		...
		/* Routes one message. Never throws and never calls send(): anything
		   it wants to say to the client goes through Client::queue().
	...
	private:
		...
		/* PASS, NICK and USER build the registration; CAP is part of the
		   handshake of every modern client; QUIT, PING and PONG must stay
		   reachable so a half registered client can leave or answer a
		...
		/* Copying the table would be pointless: Server holds exactly one
		   dispatcher for its whole lifetime. */
